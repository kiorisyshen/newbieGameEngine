    ...
    if (m_pTransform) {
        ...
        switch (time_curve_type) {
            ...
            case CurveType::kBezier:
                proportion = dynamic_pointer_cast<Bezier<float, float>>(m_Time)->Reverse(time_point, index);
                break;
            default:
                assert(0);
        }
        ...
        switch (value_curve_type) {
            case CurveType::kLinear:
                switch (m_kTrackType) {
                    case SceneObjectTrackType::kScalar: {
                        auto new_val = dynamic_pointer_cast<Linear<float, float>>(m_Value)->Interpolate(proportion, index);
                    ...
                    case SceneObjectTrackType::kQuoternion: {
                        auto new_val = dynamic_pointer_cast<Linear<Quaternion<float>, float>>(m_Value)->Interpolate(proportion, index);
                        UpdateTransform(new_val);
                        break;
                    }
                    case SceneObjectTrackType::kMatrix: {
                        auto new_val = dynamic_pointer_cast<Linear<Matrix4X4f, float>>(m_Value)->Interpolate(proportion, index);
                        UpdateTransform(new_val);
                        break;
                    }
                }
            ...
            case CurveType::kBezier:
                switch (m_kTrackType) {
                    case SceneObjectTrackType::kScalar: {
                        auto new_val = dynamic_pointer_cast<Bezier<float, float>>(m_Value)->Interpolate(proportion, index);
                        UpdateTransform(new_val);
                        break;
                    }
                    case SceneObjectTrackType::kVector3: {
                    ...
                    case SceneObjectTrackType::kQuoternion: {
                        auto new_val = dynamic_pointer_cast<Bezier<Quaternion<float>, float>>(m_Value)->Interpolate(proportion, index);
                        UpdateTransform(new_val);
                        break;
                    }
                    case SceneObjectTrackType::kMatrix: {
                        auto new_val = dynamic_pointer_cast<Bezier<Matrix4X4f, float>>(m_Value)->Interpolate(proportion, index);
                        UpdateTransform(new_val);
                        break;
                    }
                }
