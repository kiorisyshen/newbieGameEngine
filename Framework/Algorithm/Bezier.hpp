...
class Bezier : public Curve<T>
{
   ...
   public:
    ...
    T Reverse(T t) const final
    {
        ...
        if (m_Knots.size() < 2) return 0;

        if (t <= m_Knots.front()) return 0;

        if (t >= m_Knots.back()) return 1;

        for (size_t i = 1; i < m_Knots.size(); i++) {
            if (t >= m_Knots[i]) continue;
        ...
        typename NewtonRapson<T>::nr_f f = [t2, t1, c2, c1, t](T s) {
            return (t2 - 3 * c2 + 3 * c1 - t1) * pow(s, 3.0) + 3 * (c2 - 2 * c1 + t1) * pow(s, 2.0) +
                   3 * (c1 - t1) * s + t1 - t;
        };

        typename NewtonRapson<T>::nr_fprime fprime = [t2, t1, c2, c1](T s) {
            return 3 * (t2 - 3 * c2 + 3 * c1 - t1) * pow(s, 2.0) + 6 * (c2 - 2 * c1 + t1) * s + 3 * (c1 - t1);
        };
    ...
    T Interpolate(T s) const final
    {
        if (m_Knots.size() == 0)
            ...
            if (t <= m_Knots.front()) return m_Knots.front();

            if (t >= m_Knots.back()) return m_Knots.back();
            ...
            return (t2 - 3 * c2 + 3 * c1 - t1) * pow(s, 3.0f) + 3 * (c2 - 2 * c1 + t1) * pow(s, 2.0f) +
                   3 * (c1 - t1) * s + t1;
        }
    }
