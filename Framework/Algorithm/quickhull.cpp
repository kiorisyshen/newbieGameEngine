...
void QuickHull::ComputeInitialTetrahydron()
{
    ...
    PointPtr ExtremePointYMin = make_shared<Point3>(numeric_limits<float>::max());
    PointPtr ExtremePointZMin = make_shared<Point3>(numeric_limits<float>::max());
    PointPtr ExtremePointXMax = make_shared<Point3>(numeric_limits<float>::min());
    PointPtr ExtremePointYMax = make_shared<Point3>(numeric_limits<float>::min());
    PointPtr ExtremePointZMax = make_shared<Point3>(numeric_limits<float>::min());
    ...
    // now we find the most distant pair among 6 extreme points
    {
        ...
        center_of_tetrahydron = make_shared<Point3>((*A + *B + *C + *D) * 0.25f);
