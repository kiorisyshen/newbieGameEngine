...
- (void)loadMetal {
  ...
  pipelineStateDescriptor.colorAttachments[0].pixelFormat =
      _mtkView.colorPixelFormat;
  pipelineStateDescriptor.depthAttachmentPixelFormat =
      _mtkView.depthStencilPixelFormat;
...
- (void)tick:(const std::vector<std::shared_ptr<DrawBatchConstants>> &)batches {
  ...
  if (_renderPassDescriptor != nil) {
    id<MTLRenderCommandEncoder> render_encoder = [_commandBuffer
    ...
    [render_encoder setVertexBuffer:_uniformBuffers offset:0 atIndex:10];
    [render_encoder setFragmentBuffer:_uniformBuffers offset:0 atIndex:10];
    [render_encoder setFragmentSamplerState:_sampler0 atIndex:0];

    for (const auto &pDbc : batches) {
      const MtlDrawBatchContext &dbc =
          dynamic_cast<const MtlDrawBatchContext &>(*pDbc);
      ...
      for (uint32_t bufferIndex = 0; bufferIndex < dbc.property_count;
           bufferIndex++) {
      ...
      if (dbc.property_count <= 2) {
        ...
        [render_encoder setVertexBuffer:vertexBuffer offset:0 atIndex:2];
      }
...
- (void)setPerBatchConstants:
    (const std::vector<std::shared_ptr<DrawBatchConstants>> &)batches {
  for (const auto &pDbc : batches) {
